...
class PokemonMapper {
  ...
  #buildPokemonEvolutionChain({ species, is_baby, evolves_to }) {
    ...
    return stack.map(({ name, is_baby }) => `<li>${name}${is_baby ? svgBaby : ""}</li>`).join('');
  }
  #buildPokemonLocation(pokemonLocation) {
    const locationList = pokemonLocation.map(({ location_area, version_details }) => {
      let counter = 0
  ...
  #buildPokemonAbilities({abilities}) {
    const abilitiesList = abilities.map(({ ability, is_hidden }) => `<li>${ability.name}${is_hidden ? svgEye :""}</li>`);
